...
def parse_args():
    # Unchanged
    parser = argparse.ArgumentParser(description="Test Stage 2 SFT Model for Event Type Prediction Accuracy")
    ...
    parser.add_argument("--image_path_prefix", type=str, required=True, help="Global prefix to prepend to all image paths from JSON files.")
...
# ==================>> ★★★ MODIFIED: Prompt Formatter matching Event Type training ★★★ <<==================
def format_prompt_with_token_budget(potential_history_events, processor, image_path_prefix, max_seq_length, min_hist_events):
    ...
    prompt_parts, prompt_image_paths = [], []
    ...
    for hist_event in reversed(potential_history_events):
        try:
            ...
            event_type = tpp_attributes.get('event_type')  # Event type (0-5)
    ...
    TARGET_IMAGE_SIZE = # TODO: add image size
    loaded_images = []
    for path_str in prompt_image_paths:
        try:
            img = Image.open(path_str).convert('RGB').resize(TARGET_IMAGE_SIZE, Image.Resampling.LANCZOS)
    ...
    professional_system_message = (
        ...
        "Your task is to predict the type of the next taxi event in Manhattan based on a sequence of past events. "
        ...
        "Analyze the patterns in timing and location to forecast the next action. Your response must be a single token representing one of the six event types."
    )
    system_prompt = f"{IM_START}system\n{professional_system_message}{IM_END}\n"
    user_content = prompt_prefix_content + "".join(prompt_parts) + EVENT_TYPE_PREDICTION_TOKEN
    user_prompt = f"{IM_START}user\n{user_content}{IM_END}\n"
    assistant_prefix = f"{IM_START}assistant\n"
    final_prompt_text = system_prompt + user_prompt + assistant_prefix

    try:
        inputs = processor(text=[final_prompt_text], images=[loaded_images], return_tensors="pt", padding=False)
...
if __name__ == "__main__":
    args = parse_args()
    if "/path/to/your" in args.trained_model_path:
    ...
    essential_tokens = [START_OF_EVENT, IM_START, IM_END, EVENT_TYPE_PREDICTION_TOKEN, TYPE_START_TOKEN]
    ...
    logger.info("Starting evaluation loop for event type prediction accuracy...")
        ...
        for data_idx, sequence_data in enumerate(outer_loop_tqdm):
            ...
            for i in inner_loop_tqdm:
                ...
                try:
                    ...
                    match = re.search(r"<\|type_(\d+)\|>", generated_text)
                    if match:
                        predicted_type = int(match.group(1))
                    else:
                        predictions_failed_parsing += 1
                        logger.warning(f"Seq {data_idx}, Target {i}: Failed to parse type token from '{generated_text}'.")
                except Exception as e:
                ...
                gt_type = -1
                try:
                    gt_type = int(target_event['TPP_attribute']['event_type'])
                except (KeyError, TypeError, ValueError) as e:
                    logger.error(f"Invalid GT type structure (Seq {data_idx}, Target {i}): Err: {e}. GT set to invalid.")

                if gt_type != -1:
                    all_gt_types.append(gt_type)
                    all_pred_types.append(predicted_type)
    ...
    if not all_gt_types:
        logger.error("No valid ground truth types collected."); sys.exit(1)

    gts_array = np.array(all_gt_types)
    preds_array = np.array(all_pred_types)
    
    valid_indices = preds_array != -1
    ...
    if len(valid_gts) == 0:
        ...
        logger.info(f"Calculating metrics over {len(valid_gts)} valid prediction pairs (out of {len(gts_array)} total ground truth points).")
        ...
        correct_predictions = np.sum(valid_gts == valid_preds)
        total_valid_predictions = len(valid_gts)
        accuracy = correct_predictions / total_valid_predictions
        
        logger.info(f"========================================")
        logger.info(f"Correct Predictions: {correct_predictions}")
        logger.info(f"Total Valid Predictions: {total_valid_predictions}")
        logger.info(f"Overall Accuracy: {accuracy:.4f}")
        logger.info(f"========================================")

    logger.info("Saving predictions and ground truth values...")
    save_dict = {'ground_truth_types': gts_array.tolist(), 'predicted_types': preds_array.tolist()}
    save_path = 'prediction_results_event_type_accuracy.json'
    try:
    ...
    logger.info("Event Type Accuracy testing script finished.")
