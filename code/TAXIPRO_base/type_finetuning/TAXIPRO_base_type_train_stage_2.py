...
# ==================>> ★★★ MODIFIED Preprocess Function (Targeting Event Type) ★★★ <<==================
def preprocess_sft_function(example, tokenizer, min_hist_events, max_seq_length, image_path_prefix):
    ...
    This version is modified to predict the EVENT TYPE instead of the time interval.
    """
    ...
    # --- ★★★ MODIFICATION 1: Build the Response for Event Type Prediction ★★★ ---
    try:
        target_event_type = target_event.get('TPP_attribute', {}).get('event_type')
        if target_event_type is None:
            return {"sft_samples": []} # Skip if target event has no type

        # The response is now the single token representing the event type
        response_content = f"<|type_{int(target_event_type)}|>"
    except (ValueError, TypeError, KeyError):
        return {"sft_samples": []}

    # --- ★★★ MODIFICATION 2: Update System Prompt and User Prompt for New Task ★★★ ---
